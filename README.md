...
Перенесіть обидва рядки з результату до `.env`, а потім приберіть пароль із
...
На сервері створіть `stream-lab/.env` з `OWNER_PASSWORD_HASH`, `SESSION_SECRET` та
іншими значеннями з `.env.example`. Каталог `data/` повинен бути доступним для
запису користувачу з UID 1000.
